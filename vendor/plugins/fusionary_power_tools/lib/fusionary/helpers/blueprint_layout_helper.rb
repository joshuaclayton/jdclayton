module Fusionary
  ...
  module Helpers
    ...
    #     BLUEPRINT_EXTRA_CONTENT_WIDTH = 0
    #     BLUEPRINT_WIDTH =               BLUEPRINT_CONTENT_WIDTH + BLUEPRINT_EXTRA_CONTENT_WIDTH
    #     
    ...
    module BlueprintLayoutHelper
      ...
      # This is the helper for creating Blueprint-structured layouts for sidebars; only needed on layouts/*.rhtml or .erb files
      # 
      # ==== Options
      # * <tt>size</tt> -- Either a number or a symbol (key) from BlueprintLayoutHelper::MULTIPLES
      # * <tt>args</tt> -- Additional options are all all options ActionView::Helpers::TagHelper::content_tag accepts
      def sidebar_column(size="", *args, &block)
        @blueprint_grid_width ||= ApplicationController::BLUEPRINT_EXTRA_CONTENT_WIDTH
        col(size, *args, &block)
      end
      ...
      def col(size="", *args, &block)
        ...
        if block_given? && block_is_within_action_view?(block)
          concat(output_tag, block.binding)
      ...
      def header(*args, &block)
        ...
        concat(content_tag("h#{size}", capture(&block), options) + (args.include?(:with_rule) ? "<hr/>" : ""), block.binding)
      end
      ...
      def container(options = {}, &block)
        ...
        concat(content_tag(:div, capture(&block), {:class => 'container'}.merge(options)), block.binding)
      end
